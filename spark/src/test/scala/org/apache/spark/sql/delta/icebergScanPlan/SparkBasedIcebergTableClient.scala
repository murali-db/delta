...
/**
 * Implementation of IcebergTableClient that uses Spark SQL with input_file_name()
 * to discover the list of files in a table. This allows end-to-end testing without
 * a real Iceberg REST server.
 *
...
/**
 * Factory for creating SparkBasedIcebergTableClient instances.
 */
class SparkBasedIcebergTableClientFactory extends IcebergTableClientFactory {
  override def createClient(spark: SparkSession): IcebergTableClient = {
    new SparkBasedIcebergTableClient(spark)
  }
}
